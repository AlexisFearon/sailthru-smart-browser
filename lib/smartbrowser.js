...
SmartBrowser.prototype.parseURLInput = function(text,tabSetting){
    ...
    function setParams(elem,ind,arr){
        ...
        if (ind === 0){
            ...
            this.input.client_id = parseInt(elem.substring(2));
        }else if (c === "e:"){
            this.input.email = elem.substring(2);
        }else if (c === "f:"){
...
SmartBrowser.prototype.extractJSON = function(text){
	...
	if (this.isJSON(jsonStr)){
		this.input.json = JSON.parse(jsonStr);
	}
...
SmartBrowser.prototype.jumpToZendeskCase = function(input){
	var baseURL = "https://sailthru.zendesk.com/tickets/";
	var caseID = input.number;

	this.navigate(baseURL + caseID);
};

SmartBrowser.prototype.jumpToFogbugzCase = function(input){
	var baseURL = "https://sailthru.fogbugz.com/f/cases/";
	var caseID = input.number;
...
SmartBrowser.prototype.jumpToMYPage = function(input){
	...
	switch(input.category)
		...
		case "blast":
		case "blasts":
			view = input.option ? "/campaigns" : ( input.number ? "/blast" : "/campaigns");
...
SmartBrowser.prototype.jumpToSUPage = function(input){
	...
	switch(input.category)
		...
		case "message":
			view = "/lookup/message";
			input.message_id = input.option;
			["message_id"].forEach(setQuery);
			input.message_id = input.number;
			["message_id"].forEach(setQuery);
			break;
		case "profile":
			view = "/lookup/profile";
			input.lookup = input.option;
		...
		case "liststats":
			...
			input.client_id = input.number;
			["client_id"].forEach(setQuery);
			input.client_id = input.option;
			["client_id"].forEach(setQuery);
			break;
		case "fast":
		case "faststats":
			view = "/support/fast_stats";
			input.blast_ids = input.number;
			["blast_ids"].forEach(setQuery);
			input.blast_ids = input.option;
			["blast_ids"].forEach(setQuery);
			break;
		case "blast":
		case "blasts":
		...
		case "transactionalstats":
			...
			input.client_ids = input.option;
			["client_ids"].forEach(setQuery);
			input.client_ids = input.number;
			["client_ids"].forEach(setQuery);
			break;
		case "sailcast":
			view = "/sailcast";
...
SmartBrowser.prototype.jumpToDBQuery = function(input){
	var baseURL = "https://su.sailthru.com/lookup/db";
	var params = {};

	function setCriteria(elem,ind,arr){
		if (input[elem]){
			params[elem] = input[elem];
		}
	}

	input.collection = input.category;

	["collection","sort","limit","fields"].forEach(setCriteria);
	
	if (input.json || input.email || input.client_id){
		var j = input.json ? input.json : {};

		["client_id","email"].forEach(addToJSON);

		function addToJSON(elem,ind,arr){
			if (input[elem]){
				j[elem] = input[elem];
			}
		}
		params.query = JSON.stringify(j);

	} else {
		["number","option"].forEach(setQuery);

		function setQuery(elem,ind,arr){
			if (input[elem]){
				params.query = input[elem];
			}
		}
	}

	this.navigate(baseURL + this.setQueryParams(params));
};
