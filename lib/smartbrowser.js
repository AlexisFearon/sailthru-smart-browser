...
SmartBrowser.prototype.parseURLInput = function(text,tabSetting){
    ...
    function setParams(elem,ind,arr){
        ...
        if (ind === 0){
        	...
        	this.input.fullText += encodeURIComponent(" " + elem);
        }
    }
...
SmartBrowser.prototype.routeNavigation = function(input){
	...
	switch(input.command)
		...
		case "db":
			...
			break;
		case "docs":
			this.jumpToDocs(input);
			break;
		case "kb":
			this.jumpToSearch(input);
			break;
		}
};
...
SmartBrowser.prototype.jumpToDocs = function(input){
	var baseURL = "http://getstarted.sailthru.com/";
	var caseID = input.category ? "?s=" + input.category + input.fullText : "";

	this.navigate(baseURL + caseID);
};

SmartBrowser.prototype.jumpToSearch = function(input){
	var baseURL = "https://sailthru.zendesk.com/hc/en-us/search";
	var caseID = input.category ? "?query=" + input.category + input.fullText : "";
...
SmartBrowser.prototype.jumpToDBQuery = function(input){
	...
	input.client_id = parseInt(input.client_id);
